...
Instantaneous clearance alone cannot distinguish obstacles with similar separation but different relative motion. LP-Nav therefore uses a finite-horizon three-dimensional velocity-obstacle (3D-VO) formulation [7] to quantify dynamic motion conflicts. For a transition from `x_t` to `x_(t+1)` induced by action `a_t`, the model is evaluated from the resulting state and realized UAV velocity at `t+1`. To represent obstacle geometry without treating all interactions as planar, an obstacle `i` of size `(w_x_i, w_y_i, h_i)` is approximated by an overlapping vertical stack of spherical primitives. Each sphere has radius `b_i = 0.5*sqrt(w_x_i^2 + w_y_i^2)`, adjacent centers are separated by `l_i = max(w_x_i, w_y_i)`, and the stack contains `K_i = ceil(h_i/l_i)` spheres. For `j in {0,...,K_i-1}`, the center of sphere `(i,j)` is

`p^o_(i,j,t) = p^o_(i,t) + [0, 0, (j-(K_i-1)/2)*l_i]^T`,

where `p^o_(i,t)` is the obstacle center. The stack is therefore centered vertically on the original obstacle, and all of its spheres inherit the obstacle velocity `v^o_(i,t)`.

Before collision evaluation, LP-Nav masks vertically irrelevant sphere primitives from the current TTC calculation. For sphere `(i,j)` with post-transition relative height `r_(i,j,t+1)^z` and vertical relative velocity `u_(i,j,t+1)^z`, its predicted height interval is bounded by `r_(i,j,t+1)^z` and `r_(i,j,t+1)^z + H*u_(i,j,t+1)^z`. The sphere is retained only if this interval intersects the vertical relevance band `[-(b_i + m_h), b_i + m_h]`, where the height margin is chosen such that `m_h >= m_z`. This screening band therefore contains the vertical collision band `[-(b_i + m_z), b_i + m_z]` and does not discard a primitive solely because of a narrower vertical prefilter. For each retained sphere, let `r_(i,j,t+1) = p^o_(i,j,t+1) - p_(t+1)` and `u_(i,j,t+1) = v^o_(i,t+1) - v_(t+1)` denote its relative position and velocity with respect to the UAV. The sphere is inflated using horizontal and vertical safety scales `d_xy_(i,j) = b_i + m_xy` and `d_z_(i,j) = b_i + m_z`, where `m_xy` and `m_z` approximate the horizontal and vertical extents of the UAV collision envelope. These scales are summarized by `D_(i,j) = diag(d_xy_(i,j), d_xy_(i,j), d_z_(i,j))`. Under constant relative velocity, its finite-horizon velocity obstacle is defined as `VO^H_(i,j,t+1) = {v in R^3 | there exists s in (0,H] such that ||D_(i,j)^(-1)[r_(i,j,t+1) + s(v^o_(i,t+1) - v)]||_2 <= 1}`.

LP-Nav evaluates whether the realized UAV velocity `v_(t+1)` belongs to `VO^H_(i,j,t+1)`, rather than explicitly constructing the complete set or selecting and projecting a new velocity. Define the scaled relative quantities

`r_bar_(i,j,t+1) = D_(i,j)^(-1)*r_(i,j,t+1),  u_bar_(i,j,t+1) = D_(i,j)^(-1)*u_(i,j,t+1)`.

Substituting `v_(t+1)` yields the boundary-intersection condition `A_(i,j,t+1)*s^2 + B_(i,j,t+1)*s + C_(i,j,t+1) = 0`, where `A_(i,j,t+1) = dot(u_bar_(i,j,t+1), u_bar_(i,j,t+1))`, `B_(i,j,t+1) = 2*dot(r_bar_(i,j,t+1), u_bar_(i,j,t+1))`, and `C_(i,j,t+1) = dot(r_bar_(i,j,t+1), r_bar_(i,j,t+1)) - 1`. For a non-overlapping sphere, a valid time to collision exists when the relative motion is nonzero and approaching, the discriminant `Delta_(i,j,t+1) = B_(i,j,t+1)^2 - 4*A_(i,j,t+1)*C_(i,j,t+1)` is nonnegative, and the first root `T_(i,j,t+1) = (-B_(i,j,t+1) - sqrt(Delta_(i,j,t+1)))/(2*A_(i,j,t+1))` lies within `(0, H]`.

A sphere with `C_(i,j,t+1) <= 0` is already inside the inflated region and is assigned unit risk. A valid in-horizon conflict receives the exponentially decaying TTC-dependent urgency score `rho_(i,j,t+1) = exp(-T_(i,j,t+1)/tau)`, while motion with no conflict within the horizon receives zero risk. The overall post-transition risk is `rho_VO_(t+1) = max_(i,j) rho_(i,j,t+1)` over the retained sphere primitives. This aggregation emphasizes the most imminent conflict, keeps the risk bounded in `[0, 1]`, and prevents its magnitude from scaling linearly with the number of sphere primitives. The scalar TTC risk is computed from simulator states only during training and is not included in the policy observation. Its incorporation into policy learning is described in the following section, and it introduces no additional deployment-time computation beyond NavRL's existing actor-and-shield pipeline. The formulation assumes constant relative velocity within the finite horizon and provides a training-time risk measure rather than a formal safety guarantee.
...
LP-Nav organizes policy learning around a shared predictive representation. The heterogeneous observation is first processed by modality-specific encoders: a convolutional network extracts LiDAR features, while an MLP encodes dynamic-obstacle information. These features are fused with the navigation state into `z_t`. Rather than using `z_t` only for instantaneous policy and value estimation, the actor, critic, and multi-horizon outcome decoder are optimized jointly around this common representation. The actor generates the bounded Beta policy described in Sec. B.3, while the critic produces the observation-conditioned value estimate `V_psi(o_t) = V_psi(z_t)`; it does not receive the latent environment state. The outcome-learning pathway further requires `z_t` to retain information about the short-horizon consequences of the selected action.

Specifically, **multi-horizon action-conditioned outcome learning** couples `z_t` with the normalized rollout action and predicts four outcomes for each horizon `h` in `H = {1, 3, 5}`:

`y_hat_t^(h) = g_phi^(h)(z_t, a_tilde_t) = [c_hat_t^(h), s_hat_t^(h), f_hat_t^(h), p_hat_t^(h)]`.

Here, `c_hat_t^(h)` and `s_hat_t^(h)` are logits for collision and persistent-stuck events, while `f_hat_t^(h)` and `p_hat_t^(h)` predict minimum forward clearance and accumulated goal progress. Their targets are constructed directly from the collected transition sequence:

`c_t^(h) = max_(1<=k<=h) c_(t+k),  s_t^(h) = max_(1<=k<=h) s_(t+k),`

`f_t^(h) = min_(1<=k<=h) f_(t+k),  p_t^(h) = sum_(k=1)^h Delta d_(t+k),`

with all aggregations truncated at episode termination. Thus, the event targets supervise future safety failures, whereas clearance and progress capture local traversability and goal-directed motion. Only the current action `a_tilde_t` is supplied to the predictor; subsequent actions are those generated by the rollout policy and are not explicit predictor inputs. The multi-step outputs should therefore be interpreted as expected outcomes of the current action under the policy continuation, rather than predictions for a prescribed future action sequence.

The predictive objective averages equally over the three horizons:

`L_MH = (1/|H|)*sum_(h in H) [BCEWithLogits(c_hat_t^(h), c_t^(h)) + BCEWithLogits(s_hat_t^(h), s_t^(h)) + SmoothL1(f_hat_t^(h), symlog(f_t^(h))) + SmoothL1(p_hat_t^(h), symlog(p_t^(h)))]`,

where `symlog(x) = sign(x)log(1+|x|)` limits the scale variation of the continuous targets. Policy optimization and outcome learning are integrated through the joint objective

`L = L_PPO + lambda_MH*L_MH`,

with `lambda_MH = 0.1`. The rollout targets and sampled action are treated as fixed inputs. Consequently, `L_MH` is backpropagated through the outcome decoder into the same observation encoders that support the actor and critic, although it has no direct gradient path to their output heads. The coupling is therefore established at the representation level: PPO trains `z_t` to support action selection and observation-conditioned value estimation, while the multi-horizon objective simultaneously makes it predictive of collision risk, trapping, clearance, and progress. The actor consequently operates on a feature space shaped by both return maximization and explicit short-horizon outcome supervision.

This design is a predictive actor-critic representation rather than a recurrent or generative world model: it neither reconstructs future observations nor performs latent rollouts for action selection. The outcome decoder is required only to impose the predictive training objective; during policy execution, its learned information is retained through the shared encoder and consumed by the actor without evaluating the decoder itself. It therefore adds no online inference cost. PPO combines a clipped policy objective, generalized advantage estimation, a clipped value loss, and entropy regularization. Training further follows a stage-wise curriculum that progressively increases dynamic-interaction density and introduces more challenging large non-convex structures, strengthening both dynamic-obstacle avoidance and navigation around confining geometry while retaining previously acquired behavior.
