...
namespace mapManager{
	...
	void occMap::initParam(){
		if (not this->nh_.getParam(this->ns_ + "/map_frame", this->mapFrame_)){
			this->mapFrame_ = "map";
		}
		cout << this->hint_ << ": Map frame: " << this->mapFrame_ << endl;
		...
		// Sensor pose in the vehicle body frame.  For LiDAR deployments the
		// six-value form is easier to audit and override from a launch file:
		// [x, y, z, roll, pitch, yaw], with distances in metres and angles in
		// radians.  Keep the original 4x4 camera matrix as a compatibility
		// fallback for existing RGB-D launch files.
		std::vector<double> body2SensorVec;
		if (this->nh_.getParam(this->ns_ + "/body_to_sensor_xyz_rpy", body2SensorVec) &&
			body2SensorVec.size() == 6){
			const Eigen::Matrix3d rotation =
				(Eigen::AngleAxisd(body2SensorVec[5], Eigen::Vector3d::UnitZ()) *
				 Eigen::AngleAxisd(body2SensorVec[4], Eigen::Vector3d::UnitY()) *
				 Eigen::AngleAxisd(body2SensorVec[3], Eigen::Vector3d::UnitX())).toRotationMatrix();
			this->body2Cam_.setIdentity();
			this->body2Cam_.block<3, 3>(0, 0) = rotation;
			this->body2Cam_(0, 3) = body2SensorVec[0];
			this->body2Cam_(1, 3) = body2SensorVec[1];
			this->body2Cam_(2, 3) = body2SensorVec[2];
			cout << this->hint_ << ": Body-to-sensor xyz/rpy: ["
				 << body2SensorVec[0] << ", " << body2SensorVec[1] << ", " << body2SensorVec[2]
				 << ", " << body2SensorVec[3] << ", " << body2SensorVec[4] << ", "
				 << body2SensorVec[5] << "]" << endl;
		}
		else{
			std::vector<double> body2CamVec (16);
			if (not this->nh_.getParam(this->ns_ + "/body_to_camera", body2CamVec) ||
				body2CamVec.size() != 16){
				ROS_ERROR("[OccMap]: Set body_to_sensor_xyz_rpy (6 values) or body_to_camera (16 values).");
				exit(0);
			}
			for (int i=0; i<4; ++i){
				for (int j=0; j<4; ++j){
		...
		if (not this->nh_.getParam(this->ns_ + "/point_cloud_min_range", this->pointcloudMinRange_)){
			this->pointcloudMinRange_ = 0.35;
		}
		if (not this->nh_.getParam(this->ns_ + "/point_cloud_max_range", this->pointcloudMaxRange_)){
			this->pointcloudMaxRange_ = 5.0;
		}
		if (not this->nh_.getParam(this->ns_ + "/point_cloud_stride", this->pointcloudStride_)){
			this->pointcloudStride_ = 1;
		}
		if (this->pointcloudMinRange_ < 0.0 ||
			this->pointcloudMaxRange_ <= this->pointcloudMinRange_ ||
			this->pointcloudStride_ < 1){
			ROS_ERROR("[OccMap]: Invalid point-cloud range/stride configuration.");
			exit(0);
		}
		cout << this->hint_ << ": Point-cloud filter range/stride: ["
			 << this->pointcloudMinRange_ << ", " << this->pointcloudMaxRange_
			 << "] / " << this->pointcloudStride_ << endl;

		// Remove returns from the vehicle itself in the body frame (after applying
		// body2Cam_ to each sensor point).  This is deliberately an axis-aligned
		// box rather than a larger
		// radial blind zone: close obstacles outside the vehicle envelope remain
		// visible to the mapper.
		if (not this->nh_.getParam(this->ns_ + "/self_filter_enabled", this->selfFilterEnabled_)){
			this->selfFilterEnabled_ = false;
		}
		this->selfFilterMin_.setZero();
		this->selfFilterMax_.setZero();
		if (this->selfFilterEnabled_){
			std::vector<double> selfFilterMinVec;
			std::vector<double> selfFilterMaxVec;
			if (not this->nh_.getParam(this->ns_ + "/self_filter_min", selfFilterMinVec) ||
				not this->nh_.getParam(this->ns_ + "/self_filter_max", selfFilterMaxVec) ||
				selfFilterMinVec.size() != 3 || selfFilterMaxVec.size() != 3){
				ROS_ERROR("[OccMap]: self_filter_min/max must each contain three values.");
				exit(0);
			}
			for (int i=0; i<3; ++i){
				this->selfFilterMin_(i) = selfFilterMinVec[i];
				this->selfFilterMax_(i) = selfFilterMaxVec[i];
			}
			if (not this->selfFilterMin_.allFinite() ||
				not this->selfFilterMax_.allFinite() ||
				(this->selfFilterMin_.array() >= this->selfFilterMax_.array()).any()){
				ROS_ERROR("[OccMap]: self_filter_min must be finite and strictly smaller than self_filter_max on every axis.");
				exit(0);
			}
			cout << this->hint_ << ": Self-filter box: min=["
				 << this->selfFilterMin_(0) << ", " << this->selfFilterMin_(1) << ", "
				 << this->selfFilterMin_(2) << "], max=[" << this->selfFilterMax_(0) << ", "
				 << this->selfFilterMax_(1) << ", " << this->selfFilterMax_(2) << "]" << endl;
		}
		else{
			cout << this->hint_ << ": Point-cloud self-filter disabled." << endl;
		}

		if (not this->nh_.getParam(this->ns_ + "/point_cloud_sync_max_interval", this->pointcloudSyncMaxInterval_)){
			this->pointcloudSyncMaxInterval_ = 0.05;
		}
		if (not this->nh_.getParam(this->ns_ + "/point_cloud_sync_log_interval", this->pointcloudSyncLogInterval_)){
			this->pointcloudSyncLogInterval_ = 5.0;
		}
		if (not std::isfinite(this->pointcloudSyncMaxInterval_) ||
			this->pointcloudSyncMaxInterval_ <= 0.0 ||
			not std::isfinite(this->pointcloudSyncLogInterval_) ||
			this->pointcloudSyncLogInterval_ < 0.0){
			ROS_ERROR("[OccMap]: point-cloud sync max interval must be > 0 and log interval must be >= 0.");
			exit(0);
		}
		cout << this->hint_ << ": Point-cloud sync max/log interval: "
			 << this->pointcloudSyncMaxInterval_ << " / "
			 << this->pointcloudSyncLogInterval_ << " s" << endl;
		...
		this->nh_.param(this->ns_ + "/enable_static_clustering", this->enableStaticClustering_, true);
		this->nh_.param(this->ns_ + "/enable_visualization", this->enableVisualization_, true);
		this->nh_.param(this->ns_ + "/use_dynamic_obstacle_free_regions", this->useDynamicObstacleFreeRegions_, true);
		cout << this->hint_ << ": Static clustering/visualization enabled: "
			 << this->enableStaticClustering_ << "/" << this->enableVisualization_ << endl;
		cout << this->hint_ << ": Dynamic-obstacle free regions enabled: "
			 << this->useDynamicObstacleFreeRegions_ << endl;
	}
	...
	void occMap::initPrebuiltMap(){
		...
		if (pcl::io::loadPCDFile<pcl::PointXYZ> (this->prebuiltMapDir_, *cloud) == -1) //* load the file
		{
			...
			for (const auto& point: *cloud)
			{
				...
				if (not pointPos.allFinite() or not this->isInMap(pointPos)){
					continue;
				}
	...
	void occMap::registerCallback(){
		if (this->sensorInputMode_ == 0){
			...
			if (this->localizationMode_ == 0){
				...
				this->pointcloudPoseSync_->setMaxIntervalDuration(ros::Duration(this->pointcloudSyncMaxInterval_));
				...
				this->pointcloudOdomSync_->setMaxIntervalDuration(ros::Duration(this->pointcloudSyncMaxInterval_));
		...
		if (this->enableStaticClustering_){
			this->staticClusteringTimer_ = this->nh_.createTimer(ros::Duration(0.05), &occMap::staticClusteringCB, this);
		}
		...
		if (this->enableVisualization_){
			// Keep visualization on the ROS callback thread.  The previous
			// detached worker raced with point-cloud vector updates.
			this->visTimer_ = this->nh_.createTimer(ros::Duration(0.2), &occMap::visCB, this);
		}
	...
	void occMap::registerPub(){
		...
		this->mapUpdatePub_ = this->nh_.advertise<std_msgs::Header>(this->ns_+"/update", 10);
	}
	...
	void occMap::pointcloudPoseCB(const sensor_msgs::PointCloud2ConstPtr& pointcloud, const geometry_msgs::PoseStampedConstPtr& pose){
		const double syncDelta = std::abs((pointcloud->header.stamp - pose->header.stamp).toSec());
		if (pointcloud->header.stamp.isZero() || pose->header.stamp.isZero() ||
			not std::isfinite(syncDelta) || syncDelta > this->pointcloudSyncMaxInterval_){
			ROS_WARN_THROTTLE(2.0, "%s: Rejecting pointcloud/pose pair with timestamp delta %.3f ms (limit %.3f ms).",
				this->hint_.c_str(), syncDelta * 1000.0, this->pointcloudSyncMaxInterval_ * 1000.0);
			return;
		}
		if (this->pointcloudSyncLogInterval_ > 0.0){
			ROS_INFO_THROTTLE(this->pointcloudSyncLogInterval_,
				"%s: Pointcloud/pose timestamp delta %.3f ms (limit %.3f ms).",
				this->hint_.c_str(), syncDelta * 1000.0, this->pointcloudSyncMaxInterval_ * 1000.0);
		}
	...
	void occMap::pointcloudOdomCB(const sensor_msgs::PointCloud2ConstPtr& pointcloud, const nav_msgs::OdometryConstPtr& odom){
		const double syncDelta = std::abs((pointcloud->header.stamp - odom->header.stamp).toSec());
		if (pointcloud->header.stamp.isZero() || odom->header.stamp.isZero() ||
			not std::isfinite(syncDelta) || syncDelta > this->pointcloudSyncMaxInterval_){
			ROS_WARN_THROTTLE(2.0, "%s: Rejecting pointcloud/odom pair with timestamp delta %.3f ms (limit %.3f ms).",
				this->hint_.c_str(), syncDelta * 1000.0, this->pointcloudSyncMaxInterval_ * 1000.0);
			return;
		}
		if (this->pointcloudSyncLogInterval_ > 0.0){
			ROS_INFO_THROTTLE(this->pointcloudSyncLogInterval_,
				"%s: Pointcloud/odom timestamp delta %.3f ms (limit %.3f ms).",
				this->hint_.c_str(), syncDelta * 1000.0, this->pointcloudSyncMaxInterval_ * 1000.0);
		}
		// directly get the point cloud
		pcl::PCLPointCloud2 pclPC2;
	...
	void occMap::updateOccupancyCB(const ros::TimerEvent& ){
		...
		if (this->useDynamicObstacleFreeRegions_ &&
			ros::service::exists("/onboard_detector/get_dynamic_obstacles", false)){
    		ros::ServiceClient client = this->nh_.serviceClient<onboard_detector::GetDynamicObstacles>("/onboard_detector/get_dynamic_obstacles");
			onboard_detector::GetDynamicObstacles::Request req;
			onboard_detector::GetDynamicObstacles::Response res;
			req.current_position.x = this->position_(0);
			req.current_position.y = this->position_(1);
			req.current_position.z = this->position_(2);
			req.range = this->raycastMaxLength_;
			client.call(req, res);

			std::vector<geometry_msgs::Vector3> obsPos = res.position;
			std::vector<geometry_msgs::Vector3> obsVel = res.velocity;
			std::vector<geometry_msgs::Vector3> obsSize = res.size;

			std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> freeRegions;
			for (int i=0; i<int(obsPos.size()); ++i){
				Eigen::Vector3d lowerBound (obsPos[i].x-obsSize[i].x/2-0.3, obsPos[i].y-obsSize[i].y/2-0.3, std::max(0.0, obsPos[i].z-obsSize[i].z/2-0.3));
				Eigen::Vector3d upperBound (obsPos[i].x+obsSize[i].x/2+0.3, obsPos[i].y+obsSize[i].y/2+0.3, obsPos[i].z+obsSize[i].z/2+0.3);
	...
	void occMap::inflateMapCB(const ros::TimerEvent& ){
		...
		if (this->mapNeedInflate_){
			...
			std_msgs::Header update;
			update.stamp = ros::Time::now();
			update.frame_id = this->mapFrame_;
			this->mapUpdatePub_.publish(update);
		}
	}
	...
	void occMap::getPointcloud(){
		...
		this->projPoints_.reserve((this->pointcloud_.size() + this->pointcloudStride_ - 1) /
								  this->pointcloudStride_);
		Eigen::Vector3d currPointCam, currPointBody, currPointMap;
		int invalidPoints = 0;
		int selfFilteredPoints = 0;
		int rangeFilteredPoints = 0;
		for (int i=0; i<int(this->pointcloud_.size()); i += this->pointcloudStride_){
			...
			if (not currPointCam.allFinite()){
				++invalidPoints;
				continue;
			}
			currPointBody = this->body2Cam_.block<3, 3>(0, 0) * currPointCam +
				this->body2Cam_.block<3, 1>(0, 3);
			if (this->selfFilterEnabled_ &&
				(currPointBody.array() >= this->selfFilterMin_.array()).all() &&
				(currPointBody.array() <= this->selfFilterMax_.array()).all()){
				++selfFilteredPoints;
				continue;
			}
			const double sensorRange = currPointCam.norm();
			if (sensorRange < this->pointcloudMinRange_ || sensorRange > this->pointcloudMaxRange_){
				++rangeFilteredPoints;
				continue;
			}
			currPointMap = this->orientation_ * currPointCam + this->position_; // transform to map coordinate
		...
		this->projPointsNum_ = int(this->projPoints_.size());
		ROS_INFO_THROTTLE(5.0,
			"%s: Point-cloud filter kept=%d, self=%d, range=%d, invalid=%d (stride=%d).",
			this->hint_.c_str(), this->projPointsNum_, selfFilteredPoints,
			rangeFilteredPoints, invalidPoints, this->pointcloudStride_);
	}
	...
	void occMap::getMapVisData(pcl::PointCloud<pcl::PointXYZ>& mapCloud, pcl::PointCloud<pcl::PointXYZ>& inflatedMapCloud, pcl::PointCloud<pcl::PointXYZ>& exploredMapCloud, pcl::PointCloud<pcl::PointXYZ>& depthCloud){
		...
		depthCloud.header.frame_id = this->mapFrame_;
	...
	void occMap::publishProjPoints(){
		...
		cloud.height = 1;
		cloud.is_dense = true;
		cloud.header.frame_id = this->mapFrame_;
	...
	void occMap::publishMap(){
		...
		exploredCloud.header.frame_id = this->mapFrame_;
	...
	void occMap::publish2DOccupancyGrid(){
		...
		mapMsg.header.frame_id = this->mapFrame_;
	...
	void occMap::publishStaticObstacles(){
		if (this->refinedBBoxVertices_.size() != 0){
		    ...
		    line.header.frame_id = this->mapFrame_;
